...
const SintomaConsulta = require('../models/sintoma_consulta');
...
router.get('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error al obtener síntomas por consulta' });
    }
});
...
router.post('/', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error al crear síntoma por consulta' });
    }
});
