...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener pacientes", details: error.message });
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al crear paciente", details: error.message }); // Puedes incluir detalles del error
  }
});
...
router.delete("/:cedula", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al eliminar paciente", details: error.message });
  }
});
...
router.put("/:cedula", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al actualizar paciente", details: error.message });
  }
});
